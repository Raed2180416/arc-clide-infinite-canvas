...
> Historical, generator-backed prototype. It is not a current Atlas release;
> regenerate it against an explicit source cutoff before using it as evidence.
...
After generating the prototype, open `arc-clide-infinite-canvas-prototype.html`
with `?variant=A`, `?variant=B`, or `?variant=C` for:

- **A — system planes**;
- **B — causal river**; and
- **C — complete module universe**.
...
```bash
node ./generate-complete-atlas.mjs
node ./generate-infinite-canvas-prototype.mjs
```
